...
// 如果不实现Copy而选择实现Drop, 那么会出现use after free的问题:
// 因为这里的结构体里有指针, clone后的实例在被销毁时, 指针指向的内存也会被释放掉,
// 而原版还存在,只要尝试访问源头, 就会导致use after free, 最后源头被销毁时,还会进一步导致double free!
...
fn main() {
    ...
    //let buf: RawBuffer = RawBuffer::from(data);
